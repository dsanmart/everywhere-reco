...
def get_users_data(filepath="local_data/users.csv", force_download=False, cloud=False):
    """
    ...
    if not os.path.exists("local_data") and cloud == False:
        os.mkdir("local_data")

    df = maybe_download(
        filepath=filepath,
        url="https://drive.google.com/file/d/1DOz_YmB6VgLg3z6Xy9R6iYP6lVphK4o4/view?usp=share_link",
        dataset_name="users",
...
def get_events_data(
    filepath="local_data/events.csv", force_download=False, cloud=False
):
    """
    ...
    df = maybe_download(
        filepath=filepath,
        url="https://drive.google.com/file/d/1KehUtKFzjqGOxHMFNO9tf-MR5QRR7H9J/view?usp=share_link",
        dataset_name="events",
        force_download=force_download,
        cloud=cloud,
    )
...
def get_train_data(filepath="local_data/train.csv", force_download=False, cloud=False):
    """
...
def get_user_friends_data(
    filepath="local_data/user_friends.csv", force_download=False, cloud=False
):
    """
...
def get_event_attendees_data(
    filepath="local_data/event_attendees.csv", force_download=False, cloud=False
):
    """
